...
date_range_list_pl = era_5_datestrings(data_interval,'pl')
...
for i in range(0,len(date_range_list_pl),data_interval):
    ...
    merged.to_netcdf('/glade/scratch/acheung/intermediate/PI/merged.nc')

    ds_1 = run_sample_dataset_2('/glade/scratch/acheung/intermediate/PI/merged.nc')
    ...
    ds_1.to_netcdf('/glade/scratch/acheung/PI/'+ var_file_name_full)
    print('...PI computation complete and saved\n')

    end = time.time()
    print(f"Runtime of week {iteration} of {len(np.arange(0,len(T_path_strs),data_interval))} is {round(end - start,2)} s")
